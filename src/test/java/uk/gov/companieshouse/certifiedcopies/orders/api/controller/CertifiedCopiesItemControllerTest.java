...
class CertifiedCopiesItemControllerTest {
    ...
    void getCertifiedCopyItemIsPresent() {
        when(certifiedCopyItemService.getCertifiedCopyItemById(ID, entitledToFreeCertificates)).thenReturn(Optional.of(item));
    ...
    void getCertifiedCopyItemNotFound() {
        when(certifiedCopyItemService.getCertifiedCopyItemById(ID, entitledToFreeCertificates)).thenReturn(Optional.empty());
        ResponseEntity<Object> response = controllerUnderTest.getCertifiedCopy(ID, REQUEST_ID_VALUE, request);
